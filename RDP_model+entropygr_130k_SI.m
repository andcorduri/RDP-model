...
ShowLegend[Plot[{CDF[LogNormalDistribution[\[Mu],\[Sigma]],x]/.soldist/.{C[1]->0,C[2]->0}/.{Mn->113.950,Mw->130.414},CDF[distD,x]},{x,0,250},Frame->True,ImageSize->600,BaseStyle->{FontSize->21,FontFamily->"Arial"},FrameLabel->{"M, kDa","Cumulative probability"},PlotStyle->{Black,{Red,Dashed}},AspectRatio->0.8,Exclusions->None],Legend1]
...
\[Tau]eIn=3.5*10^-3;(*Rouse relaxation time of one entanglement
...
(* ::Input::Initialization:: *)
sol\[Epsilon]dp01=RDPmodel[0.01,mIn,MwRDP,MeIn,\[Tau]eIn,GN0In,\[Lambda]maxIn,{10^-2,3.5*10^2},Table[{1,1,1},{i,1,Length[MwRDP]},{j,1,Length[MwRDP]}]];
...
(* ::Input::Initialization:: *)
sol\[Epsilon]dp01c=RDPmodel[0,mIn,MwRDP,MeIn,\[Tau]eIn,GN0In,\[Lambda]maxIn,{10^-2,2*10^2},Last[sol\[Epsilon]dp01]];
...
(* ::Input::Initialization:: *)
sol\[Epsilon]dp01r=RDPmodel[-0.01,mIn,MwRDP,MeIn,\[Tau]eIn,GN0In,\[Lambda]maxIn,{10^-2,2*10^2},Last[sol\[Epsilon]dp01]];
...
(* ::Input::Initialization:: *)
soltr2=Table[{cxx,RDPmodelC2[2,cxx,MwRDP,MeIn,\[Tau]eIn,GN0In,\[Lambda]maxIn]},{cxx,0.1,1,0.01}];
soltr2p1=Table[{cxx,RDPmodelC2[2.1,cxx,MwRDP,MeIn,\[Tau]eIn,GN0In,\[Lambda]maxIn]},{cxx,0.1,1,0.01}];
soltr2p2=Table[{cxx,RDPmodelC2[2.2,cxx,MwRDP,MeIn,\[Tau]eIn,GN0In,\[Lambda]maxIn]},{cxx,0.1,1,0.01}];
soltr2p5=Table[{cxx,RDPmodelC2[2.5,cxx,MwRDP,MeIn,\[Tau]eIn,GN0In,\[Lambda]maxIn]},{cxx,0.1,1,0.01}];
soltr2p6=Table[{cxx ,RDPmodelC2[2.6,cxx,MwRDP,MeIn,\[Tau]eIn,GN0In,\[Lambda]maxIn]},{cxx,0.1,1,0.01}];
soltr2p7=Table[{cxx,RDPmodelC2[2.7,cxx,MwRDP,MeIn,\[Tau]eIn,GN0In,\[Lambda]maxIn]},{cxx,0.1,1,0.01}];
soltr4=Table[{cxx,RDPmodelC2[4,cxx,MwRDP,MeIn,\[Tau]eIn,GN0In,\[Lambda]maxIn]},{cxx,0.1,1,0.01}];
